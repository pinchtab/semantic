...
# Login page
result=$(semantic find "sign in button" --snapshot /testdata/snapshots/login-page.json --format json)
...
result=$(semantic find "email input" --snapshot /testdata/snapshots/login-page.json --format json)
assert_json_field "$result" ".best_ref" "e1" "login: email input → e1"

result=$(semantic find "password field" --snapshot /testdata/snapshots/login-page.json --format json)
assert_json_field "$result" ".best_ref" "e2" "login: password field → e2"

# Ecommerce
result=$(semantic find "add to cart" --snapshot /testdata/snapshots/ecommerce-product.json --format json)
...
result=$(semantic find "quantity" --snapshot /testdata/snapshots/ecommerce-product.json --format json)
assert_json_field "$result" ".best_ref" "e8" "ecommerce: quantity → e8"

# Dashboard
result=$(semantic find "search box" --snapshot /testdata/snapshots/dashboard.json --format json)
assert_json_field "$result" ".best_ref" "e6" "dashboard: search box → e6"

result=$(semantic find "log out" --snapshot /testdata/snapshots/dashboard.json --format json)
